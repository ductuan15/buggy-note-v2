...
public class TrashFragment extends Fragment {
    ...
    private FragmentTrashBinding binding = null;

    private NotesViewModel notesViewModel;

    private final TrashActionMode actionModeCallback = new TrashActionMode();
    ...
    private final NoteAdapter trashedNoteAdapter = new NoteAdapter(noteAdapterCallbacks, NoteAdapter.TRASH_TAG);
    ...
    private void invalidateCab() {
        ...
        int nSelectedItems = trashedNoteAdapter.numberOfSelectedItems();
        if (nSelectedItems == 0) {
            actionModeCallback.finishActionMode();
            return;
        }
        ...
        trashedNoteAdapter.enableSelection();
    }
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        NoteItemTouchHelperCallback callback = new NoteItemTouchHelperCallback(trashedNoteAdapter);
    ...
    void initRecyclerViews() {
        initNoteAdapter(binding.noteList, trashedNoteAdapter, noteListTouchHelper, true);
        ...
        notesViewModel.getNavigateToNoteDetails().observe(getViewLifecycleOwner(), id -> {
            if (id != null) {
                Navigation.findNavController(requireActivity(), R.id.nav_host_fragment).navigate(
                        TrashFragmentDirections.actionNavNotesToNoteDetailsFragment(id)
                );
    ...
    private void refreshNoteList() {
        ...
        initNoteAdapter(binding.noteList, trashedNoteAdapter, noteListTouchHelper, false);
    ...
    class TrashActionMode implements ActionMode.Callback {
        ...
        public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
            ...
            mode.setTitle(Integer.valueOf(trashedNoteAdapter.numberOfSelectedItems()).toString());
            return true;
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            switch (item.getItemId()) {
                case R.id.action_restore: {
                    ...
                    List<NoteWithTags> selectedItems = new ArrayList<>(trashedNoteAdapter.getSelectedItems());
                ...
                case R.id.action_select_all: {
                    ...
                    trashedNoteAdapter.selectAll();
                    invalidateCab();

                    return true;
                }

                case R.id.action_remove_forever: {
                    List<NoteWithTags> selectedTrashedItems = trashedNoteAdapter.getSelectedItems();

                    List<NoteWithTags> selectedItems = new ArrayList<>(trashedNoteAdapter.getSelectedItems());

                    if (selectedTrashedItems.isEmpty()) {
                        return true;
                    }
                    ...
                    new MaterialAlertDialogBuilder(requireContext())
                            ...
                            .setPositiveButton(getString(R.string.remove), (v, u) -> {
                                notesViewModel.permanentlyRemoveNote(selectedItems);
                                actionModeCallback.finishActionMode();
                            })
                            .show();
                    return true;
